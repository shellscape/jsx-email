...
export const handleAnchorHighlighting = (props: AnchorProps) => {
  ...
  for (let i = anchors.length - 1; i >= 0; i--) {
    ...
    if (anchorTop < props.scrollTop + props.viewportHeight && anchorTop + 75 > props.scrollTop) {
      activeAnchors.push(anchors[i].id);
    }
  }

  const closestAnchor = anchors.find((anchor) => anchor.offsetTop > props.scrollTop);

  if (closestAnchor && closestAnchor.offsetTop - 75 > props.scrollTop) {
    const index = anchors.findIndex((anchor) => anchor.offsetTop === closestAnchor.offsetTop);
    if (index !== -1) {
...
export const updateDialectLinks = () => {
  ...
  linksWithDialects.forEach((link) => {
    const { href } = (link as HTMLAnchorElement).dataset;
