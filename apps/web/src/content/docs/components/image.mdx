...
type: component
copyPromptComponent: Img
copyPrompt: Use for hosted email images. Prefer absolute PNG/JPG/GIF URLs, avoid SVGs, provide meaningful `alt`, and set explicit `width` and `height`.
---
...
<Callout type="info" title="Tips">
  All email clients can display `.png`, `.gif`, and `.jpg` images. Unfortunately, `.svg` images are
  not well supported, regardless of how they're referenced, so avoid using these. See [Can I
