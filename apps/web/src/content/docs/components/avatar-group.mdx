...
type: component
copyPrompt: Use only to arrange `Avatar` children. Use `max` for long groups, `direction` for RTL layouts, and keep `overlap`/`spacing` conservative because negative margins vary by client.
---
