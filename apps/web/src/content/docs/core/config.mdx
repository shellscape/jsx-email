...
_Optional_. Allows the configuration file to specify the default email clients used by the CLI's `check` command. The `--email-clients` flag takes precedence when provided.
...
<Callout type="info" title="Tips">
  ESBuild plugins are only run when using the CLI's `build` or `preview` commands. ESBuild, and by
  extension the `esbuild` configuration option, are not used when using `render` directly in code
</Callout>
