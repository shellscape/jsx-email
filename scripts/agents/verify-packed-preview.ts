...
async function main() {
  ...
  const result = await page.evaluate(() => {
    ...
    const mainScript = document.querySelector<HTMLScriptElement>('script[src="/main.tsx"]');
    const inlineStyle = document.querySelector('style')?.textContent || '';

    return {
      fileCount: fileCount?.[0] || null,
      hasInlineStyles: inlineStyle.includes('tailwindcss') && inlineStyle.includes('--bg'),
  ...
  if (!result.hasMainScript) throw new TypeError('Packed app did not load /main.tsx.');
